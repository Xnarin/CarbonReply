...
export function OriginalPdfDialog({ buttonLabel = "원본 PDF 대조", fileName, sourceUrl, usageKwh }: { buttonLabel?: string; fileName: string; sourceUrl: string; usageKwh?: number }) {
  ...
  useEffect(() => {
    ...
    async function renderEvidence() {
      ...
      try {
        ...
        const target = usageKwh === undefined ? "" : normalizeNumber(String(usageKwh));
        let matchedPage = 1;
        ...
        for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber += 1) {
          ...
          const item = target ? text.items.find((candidate) => "str" in candidate && normalizeNumber(candidate.str).includes(target)) : undefined;
          if (item && "str" in item) {
        ...
        if (matchedItem) {
          ...
          setStatus("원본 고지서를 화면 안에서 확인할 수 있습니다.");
        }
      } catch {
  ...
  return <>
    <button className="source-preview-button" onClick={() => setIsOpen(true)} type="button">{buttonLabel}</button>
