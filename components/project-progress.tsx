...
export function ProjectProgress({ activeStep, projectId: _projectId, onLogout }: { activeStep: 1 | 2 | 3 | 4; projectId?: string; onLogout?: typeof signOut }) {
  return <nav className="project-progress" aria-label="산정 진행 단계">
    ...
    {onLogout ? <div className="account-area"><span className="account-label">협력사 탄소데이터 응답</span><button className="logout-button" formAction={onLogout} type="submit">로그아웃</button></div> : null}
  </nav>;
}
