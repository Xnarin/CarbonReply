...
type ExistingProject = {
  ...
  created_at: string;
  summary?: { totalKwh: number; totalTco2e: number };
};
...
function ProjectRecords({ existingProjects }: { existingProjects: ExistingProject[] }) {
  ...
  return <section className="existing-projects">
    <div className="project-record-heading">
      <div><p className="card-kicker">PROJECT RECORDS</p><h2>{recordTab === "active" ? "진행 중인 산정" : "확정된 결과"}</h2></div>
      <div className="record-tabs" role="tablist">
        <button aria-selected={recordTab === "active"} className={recordTab === "active" ? "is-active" : ""} onClick={() => setRecordTab("active")} role="tab" type="button">진행 중 <b>{activeCount}</b></button>
        <button aria-selected={recordTab === "completed"} className={recordTab === "completed" ? "is-active" : ""} onClick={() => setRecordTab("completed")} role="tab" type="button">확정 결과 <b>{completedCount}</b></button>
      </div>
    </div>
    ...
    <div className="workbench-frame">
      ...
      <div className="workbench-body setup-body">
        <section className="workbench-main">
          ...
          {existingProjects.length > 0 ? <ProjectRecords existingProjects={existingProjects} /> : null}
        </section>
        <aside className="workbench-aside">
          <section className="status-card blueprint-frame"><CornerMarks /><p className="card-kicker">현재 단계</p><p className="setup-stage"><b>01</b><span>산정 설정</span></p><p className="card-copy">프로젝트를 만든 다음에만 고지서를 업로드합니다. 아직 파일을 선택할 필요는 없습니다.</p></section>
          <section className="factor-card"><p className="card-kicker">적용 예정 계수</p><dl><div><dt>배출계수</dt><dd>{selectedFactor.value} {selectedFactor.unit}</dd></div><div><dt>출처</dt><dd><a href={ELECTRICITY_FACTOR_SOURCE_URL} rel="noreferrer" target="_blank">EG-TIPS 원문 ↗</a></dd></div><div><dt>기준</dt><dd>{selectedFactor.factorYear}년 공개 기준</dd></div></dl>{selectedFactor.isFallback ? <p className="factor-warning">{selectedYear}년 공식 계수가 없어 최신 공개값인 {selectedFactor.factorYear}년 계수를 임시 적용합니다.</p> : null}</section>
