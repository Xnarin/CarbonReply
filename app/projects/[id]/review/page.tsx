...
export default async function ReviewPage({ params }: ReviewPageProps) {
  ...
  return <main className="review-page"><section className="review-panel">
    ...
    <section className="review-command"><div><span>확정 진행</span><b>{confirmedCount} / {rows.length}개월</b><p>{hasUnconfirmedWarning ? "주의 항목은 원본과 대조해 개별 확정한 뒤 전체 확정을 사용할 수 있습니다." : "각 사용량을 고지서와 비교한 뒤 확정하세요. 전체 확정은 현재 추출된 값을 모두 확정합니다."}</p></div><form action={confirmAllMonthlyUsage}><input name="projectId" type="hidden" value={project.id} /><ConfirmUsageButton all disabled={hasUnconfirmedWarning} /></form></section>
    ...
    <section className="review-table-section"><div className="review-table-heading"><h2>월별 사용량</h2><span>원본 고지서의 청구월과 사용량을 대조한 뒤 확정해 주세요.</span></div><div className="review-table-wrap"><table><thead><tr><th>청구월</th><th>원본 근거</th><th>전기 사용량</th><th>추정 배출량</th><th>확정</th></tr></thead><tbody>{rows.length === 0 ? <tr><td className="review-empty" colSpan={5}>먼저 전기요금 고지서를 업로드해 주세요.</td></tr> : rows.map((row) => { const evidence = evidenceByMonth.get(row.month); const isOutlier = outlierMonths.has(row.month); const isZero = zeroUsageMonths.has(row.month); return <tr className={isOutlier || isZero ? "has-validation-warning" : ""} key={row.month}><td><div className="month-validation"><span>{formatMonth(row.month)}</span>{isOutlier ? <small>평소 범위 확인</small> : isZero ? <small>0 kWh 확인</small> : null}</div></td><td>{evidence ? <div className="source-evidence"><span title={evidence.file_name}>{evidence.file_name}</span><small>추출값 {Number(evidence.parsed_kwh).toLocaleString()} kWh</small><OriginalPdfDialog fileName={evidence.file_name} sourceUrl={`/api/projects/${project.id}/uploads/${evidence.id}`} usageKwh={Number(evidence.parsed_kwh)} /></div> : <span className="manual-source">직접 입력값</span>}</td><td><form action={confirmMonthlyUsage} className="usage-form"><input name="projectId" type="hidden" value={project.id} /><input name="month" type="hidden" value={row.month} /><input aria-label={`${formatMonth(row.month)} 전기 사용량`} defaultValue={row.kwh} max="100000000" min="0" name="kwh" step="0.01" type="number" /><span>kWh</span><ConfirmUsageButton confirmed={row.confirmed} /></form></td><td>{calculateElectricityEmissionsKg(Number(row.kwh), project.target_year).toFixed(1)} kgCO₂e</td><td><span className={`review-status ${row.confirmed ? "is-confirmed" : ""}`}>{row.confirmed ? "확정됨" : "확인 필요"}</span></td></tr>; })}</tbody></table></div></section>
