...
export default async function ReviewPage({ params }: ReviewPageProps) {
  ...
  return <main className="review-page"><section className="review-panel">
    ...
    <section className="review-command"><div><span>확정 진행</span><b>{confirmedCount} / {rows.length}개월</b><p>{warningMonths.size > 0 ? "주의 항목은 원본 고지서와 대조해 필요하면 값을 수정·저장해 주세요. 저장한 월은 바로 확정됩니다." : "각 사용량을 고지서와 비교해 필요하면 수정·저장하세요. 저장한 월은 바로 확정되며, 전체 확정은 현재 값을 한 번에 반영합니다."}</p></div><ConfirmAllUsageForm allConfirmed={canCreateReport} disabled={rows.length === 0} projectId={project.id} /></section>
