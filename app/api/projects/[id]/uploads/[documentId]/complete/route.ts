...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string; documentId: string }> }) {
  ...
  const { data: document, error } = await supabase
    .from("documents")
    .update({ parse_status: "pending", parse_error_code: null, uploaded_at: new Date().toISOString() })
    .eq("id", documentId)
  ...
  try {
    ...
    if (!isValidPdfHeader(pdf)) {
      await supabase.from("documents").update({ parse_status: "failed", parse_error_code: "invalid_pdf" }).eq("id", documentId).eq("project_id", projectId);
    ...
    if (existingActivity) {
      await supabase.from("documents").update({ parse_status: "failed", parse_error_code: "duplicate_month", parsed_month: month, parsed_kwh: extracted.usageKwh }).eq("id", documentId).eq("project_id", projectId);
    ...
    await supabase.from("documents").update({ parse_status: "failed", parse_error_code: "extraction_failed" }).eq("id", documentId).eq("project_id", projectId);
